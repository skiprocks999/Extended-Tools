...
public class ExtendedTools {
  ...
  @Mod.EventBusSubscriber(value = Dist.CLIENT, modid = References.ID,
  ...
  private static class ElectroCreativeRegistry {
    ...
    public static void registerItems(BuildCreativeModeTabContentsEvent event) {
      CreativeModeTab tab = ((RegistryObject<CreativeModeTab>) DeferredRegisters.CREATIVE_TABS
          .getEntries().toArray()[0]).get();
      if (event.getTab() == tab) {
